...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/23 13:16:02 by eryilmaz          #+#    #+#             */
/*   Updated: 2022/02/23 14:02:45 by eryilmaz         ###   ########.tr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*anametin;
	static char	*yenibas[10000];

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	yenibas[fd] = okuma(fd, yenibas[fd]);
	if (!yenibas[fd])
		return (NULL);
	anametin = anametinbulucu(yenibas[fd]);
	yenibas[fd] = yenibaslangic(yenibas[fd]);
	return (anametin);
}
